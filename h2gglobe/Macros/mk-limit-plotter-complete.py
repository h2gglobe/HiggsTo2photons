...
parser.add_option("-l","--addLine",dest="addLineFile")
...
if Method == "Asymptotic":  EXPName = Method+"/higgsCombineTest."+Method  # everything contained here
...
graphOth = ROOT.TGraphAsymmErrors()
if options.addLineFile!="":
  lineFile = ROOT.TFile.Open(options.addLineFile)
  graphOth = ROOT.TGraphAsymmErrors(lineFile.Get("median"))
...
leg.AddEntry(graphMed,"Median Expected %s Limit Gradient Boost"%LegendEntry,"L")
if options.addLineFile != "": leg.AddEntry(graphOth,"Median Expected %s Limit Adaptive Boost"%LegendEntry,"L")
...
# newMG.Add(graphMed)
 ...
 graphmede.Draw("AP")
 newCanvas.SaveAs(options.path+"smoothTest.pdf")
...
if options.addLineFile != "":
  graphOth.SetLineStyle(2)
  graphOth.SetLineColor(1)
  graphOth.SetLineWidth(3)
  MG.Add(graphOth)
...
if options.doRatio:
 C.SaveAs("limit/"+"limit_%s_%s_ratio.pdf"%(args[1],Method))
 C.SaveAs("limit/"+"limit_%s_%s_ratio.gif"%(args[1],Method))
 C.SaveAs("limit/"+"limit_%s_%s_ratio.eps"%(args[1],Method))
 C.SaveAs("limit/"+"limit_%s_%s_ratio.ps"%(args[1],Method))
 C.SaveAs("limit/"+"limit_%s_%s_ratio.png"%(args[1],Method))
else:
 C.SaveAs("limit/"+"limit_%s_%s.pdf"%(args[1],Method))
 C.SaveAs("limit/"+"limit_%s_%s.gif"%(args[1],Method))
 C.SaveAs("limit/"+"limit_%s_%s.eps"%(args[1],Method))
 C.SaveAs("limit/"+"limit_%s_%s.ps"%(args[1],Method))
 C.SaveAs("limit/"+"limit_%s_%s.png"%(args[1],Method))

if options.outputLimits:
  print "Writing Limits To ROOT file"
  OUTTgraphs = ROOT.TFile("limit/"+options.outputLimits,"RECREATE")
