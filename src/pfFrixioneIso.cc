...
pfFrixioneIso::pfFrixioneIso(){
  ...
  sprintf(filename, "%s/src/HiggsAnalysis/HiggsTo2photons/data/TMVAClassification_BDT.weights.xml", descr);
  
  tmvaReaderID->BookMVA("BDT", filename);
}

pfFrixioneIso::~pfFrixioneIso() 
{}

float pfFrixioneIso::mvaID(const reco::PFCandidateCollection* pfParticlesColl,const reco::Photon *recoPhoton ,edm::Handle< reco::VertexCollection > recoVtx){
...
float pfFrixioneIso::mvaIDWithIsodeps(const reco::PFCandidateCollection* pfParticlesColl,const reco::Photon *recoPhoton, reco::PFCandidatePtr PFCandPtr, IsoDepositMaps photonIsoDep, float fastJetCorr){
...
float pfFrixioneIso::getMustacheValue(reco::PFCandidate PFCand) {
...
vector<float > pfFrixioneIso::getPFIsolationNeutral(const reco::PFCandidateCollection* pfParticlesColl,int iType, math::XYZPoint photon,bool doVeto){
  ...
  for(unsigned iPF=0; iPF<pfParticlesColl->size(); iPF++) {
    ...
    if(doVeto){
      if(abs(photonWRTPFParticleVtx.Eta()-pfParticle.eta())<0.015&&deltaPhi(photonWRTPFParticleVtx.Phi(),pfParticle.phi())<0.4)
	continue;
    }
...
vector<vector<float > > pfFrixioneIso::getPFIsolationCharged(const reco::PFCandidateCollection* pfParticlesColl,int iType, math::XYZPoint photon,bool doVeto,edm::Handle< reco::VertexCollection > recVtxs_){
  ...
  for(reco::VertexCollection::const_iterator vertex=vertices.begin(); vertex!=vertices.end(); ++vertex) {
	...
	continue;
      ...
      if(fDeltaR>0.8||fDeltaR<0.015)
	continue;

      if(doVeto){
	if(abs(photonWrtVertex.eta()-pfParticle.eta())<0.015&&deltaPhi(photonWrtVertex.phi(),pfParticle.phi())<0.4)
	  continue;
      }
...
vector<float > pfFrixioneIso::getIsolationFromIsodeps(reco::PFCandidatePtr PFCand,IsoDepositMaps photonIsoDep){
...
bool pfFrixioneIso::isType(int iType,int iPdgID){
