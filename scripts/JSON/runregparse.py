...
for DSindex in range(0,len(DS_LIST)):
    sel_temp="{groupName} ='"+GROUP+"' and {datasetState} = 'COMPLETED' and {runNumber} >= "+DS_RUNMIN[DSindex]+" and {runNumber} <= "+DS_RUNMAX[DSindex]+" and {bfield}>"+BFIELD+" and {datasetName} LIKE '"+DS_LIST[DSindex]+"'"
...
# the lumisection selection is on the Prompt dataset:
sel_dstable="{groupName} ='"+GROUP+"' and {runNumber} >= "+RUNMIN+" and {runNumber} <= "+RUNMAX+" and {bfield}>"+BFIELD+" and {datasetName} LIKE '%Prompt%'"
...
sel_dcstable="{groupName} ='"+GROUP+"' and {runNumber} >= "+RUNMIN+" and {runNumber} <= "+RUNMAX+" and {datasetName} LIKE '%Prompt%'"
for dcs in DCSLIST:
    if dcs !="NONE":
