/****************************************************************************
  ...
  This algorithm gives the size of all of the branchces and in addition print them 
  ...
  // 1. Enable macro
  .L MakeTreeStat.C+
  // 2. Print statistic (sorting according secon argument - either zip Bytes (kTRUE) or TotSize (kFALSE)
  // 3. MakeStat(int storage, "root_filename", kTRUE);
     0 disk, 1 CASTOR

  The tree name is hardcoded but can be easily modified ;-)
...
TFile     *file;           // file of interest
//TRFIOFile *rfiofile;           // file of interest
TTree     *fTree, *tree;   // tree of interest
...
void MakeTreeStat(char* filename, Bool_t zipSort);
...
void MakeTreeStat(const char *prefix, TBranch * branch, Float_t* size, Float_t mratio);

void MakeTreeStat(int storage, char* filename, Bool_t zipSort){
  ...
  if (storage == 1) {
    //  rfiofile = new TRFIOFile(filename);
    //tree = (TTree*) rfiofile->Get("event");
  }
  ...
  for (Int_t i=0; i<array->GetEntries(); i++){
    MakeTreeStat(prefix,(TBranch*)array->At(i),size, mratio);
  }  
...
void MakeTreeStat(const char *prefix, TBranch * branch, Float_t *size, Float_t mratio){
  ...
  for (Int_t i=0; i<array->GetEntries(); i++){
    ...
    TString str=prefix;
    str+= branch->GetName();
    MakeTreeStat(str.Data(),(TBranch*)array->At(i), bsize, mratio);
    bsizeSum[0]+=bsize[0];
    bsizeSum[1]+=bsize[1];
    size[0]+=bsize[0];
    size[1]+=bsize[1];
